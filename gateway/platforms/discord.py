...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.kora/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _command_sync_state_path(self) -> _Path:
        ...
        directory = get_kora_home() / _DISCORD_COMMAND_SYNC_STATE_SUBDIR
        try:
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description=f"Reset your {self.config.display_name} session")
        ...
        @tree.command(name="status", description=f"Show {self.config.display_name} session status")
        ...
        @tree.command(name="stop", description=f"Stop the running {self.config.display_name} agent")
        ...
        @tree.command(name="reload-skills", description="Re-scan ~/.kora/skills/ for new or removed skills")
        ...
        @tree.command(name="update", description=f"Update {self.config.display_name} Agent to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description=f"Gracefully restart the {self.config.display_name} gateway")
        ...
        @tree.command(name="thread", description=f"Create a new thread and start a {self.config.display_name} session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message=f"Optional first message to send to {self.config.display_name} in the thread",
        ...
        # kora_cli/commands.py automatically appear as Discord slash
        ...
        try:
            from kora_cli.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
        ...
        try:
            from kora_cli.commands import _iter_plugin_command_entries
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        try:
            ...
            cmd = discord.app_commands.Command(
                name="skill",
                description=f"Run a {self.config.display_name} skill",
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        """
        from kora_cli.commands import discord_skill_commands_by_category
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by {self.config.display_name}: **{name}**"
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        thread_name = content[:80] if content else self.config.display_name
        if len(content) > 80:
        ...
        try:
            ...
            try:
                seed_msg = await message.channel.send(f"\U0001f9f5 Thread created by {self.config.display_name}: **{thread_name}**")
                thread = await seed_msg.create_thread(
                    name=thread_name,
    ...
    async def create_handoff_thread(
        ...
        reason = f"{self.config.display_name} session handoff"
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 {self.config.display_name} handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
    ...
    async def send_clarify(
        ...
        try:
            ...
            embed = discord.Embed(
                title=f"❓ {self.config.display_name} needs your input",
                description=body,
                color=discord.Color.orange(),
            )
