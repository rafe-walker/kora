"""Centralized logging setup for the Kora runtime.

Inherited from upstream Hermes (NousResearch/hermes-agent). Provides a
single ``setup_logging()`` entry point that both the CLI and gateway
call early in their startup path.  All log files live under
``~/.kora/logs/`` (profile-aware via ``get_kora_home()``).
...
from kora_constants import get_config_path, get_kora_home
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    def __init__(self, *args, **kwargs):
        from kora_cli.config import is_managed
