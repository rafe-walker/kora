...
TIPS = [
    ...
    "SOUL.md at ~/.kora/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    ...
    "Checkpoints use shadow repos in ~/.kora/checkpoints/ — your project's .git is never touched.",
    ...
    "Cron scripts live in ~/.kora/scripts/ and run before the agent — perfect for data collection pipelines.",
    ...
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.kora/SOUL.md to customize.",
    ...
    "The skills audit log at ~/.kora/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Every interrupt during an agent run is logged to ~/.kora/interrupt_debug.log with timestamps.",
    ...
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.kora/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.kora/skills/.hub/quarantine/ holds skills pending security review.",
    ...
    '/reload reloads ~/.kora/.env into the running session — pick up new API keys without restarting.',
    ...
    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.kora/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.kora/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',
    ...
    'Drop a YAML in ~/.kora/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.kora/dashboard-plugins/ — no npm build required.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.kora/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.kora/skills/ so drop-in skills appear without restarting the session.',
    ...
    'hermes chat --ignore-user-config skips ~/.kora/config.yaml — reproducible bug reports and CI runs.',
    ...
    'HERMES_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.kora/plugins/ — trust-gated by design.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.kora/dashboard-plugins/.',
]
