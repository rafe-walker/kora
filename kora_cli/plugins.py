...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with hermes-agent;
   ...
   own discovery paths)
2. **User plugins**   – ``~/.kora/plugins/<name>/``
3. **Project plugins** – ``./.kora/plugins/<name>/`` (opt-in via
...
# stderr in addition to ~/.kora/logs/agent.log. Aimed at plugin authors
...
class PluginManifest:
    ...
    #              in ~/.kora/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
...
class PluginContext:
    ...
    def register_skill(
        ...
        ``~/.kora/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        #
        # Repo-shipped plugins live next to kora_cli/. Two layouts are
        ...
        # 2. User plugins (~/.kora/plugins/)
        user_dir = get_kora_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.kora/plugins/)
        if _env_enabled("HERMES_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".kora" / "plugins"
