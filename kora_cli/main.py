...
# IMPORTANT: kora_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``kora_bootstrap`` is a
...
# new code references ``kora_bootstrap`` but the editable install's
...
# Falls back to ~/.kora/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    ...
    # Mirrors kora_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # parent directory name (e.g. ~/.kora/profiles/coder or
    ...
    if profile_name is None:
        try:
            ...
            active_path = get_default_kora_root() / "active_profile"
    ...
    if profile_name is not None:
        try:
            from kora_cli.profiles import resolve_profile_env
...
# Load .env from ~/.kora/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from kora_cli.config import get_kora_home
from kora_cli.env_loader import load_hermes_dotenv

load_hermes_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → HERMES_REDACT_SECRETS env
# var BEFORE kora_logging imports agent.redact (which snapshots the flag at
...
try:
    if "HERMES_REDACT_SECRETS" not in os.environ:
        ...
        _cfg_path = get_kora_home() / "config.yaml"
        if _cfg_path.exists():
...
try:
    from kora_logging import setup_logging as _setup_logging
...
try:
    from kora_cli.config import load_config as _load_config_early
    from kora_constants import apply_ipv4_preference as _apply_ipv4
...
def _has_any_provider_configured() -> bool:
    ...
    from kora_cli.config import get_env_path, get_kora_home, load_config
    ...
    from kora_cli.config import DEFAULT_CONFIG
    ...
    auth_file = get_kora_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
...
def _ensure_tui_node() -> None:
    ...
    hermes_home = os.environ.get("HERMES_HOME") or str(Path.home() / ".kora")
    try:
...
def _find_bundled_tui(kora_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if kora_cli_dir is None:
        kora_cli_dir = Path(__file__).parent
    bundled = kora_cli_dir / "tui_dist" / "entry.js"
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    def _node_bin(bin: str) -> str:
        ...
        if not path and bin == "node":
            try:
                from kora_cli.dep_ensure import ensure_dependency
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from kora_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    if code == 42:
        from kora_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    ...
    if os.environ.get("HERMES_KANBAN_BOARD"):
        return
    try:
        from kora_cli.kanban_db import get_current_board
...
def cmd_chat(args):
    ...
    try:
        from kora_cli.banner import prefetch_update_check
    ...
    # user's ~/.kora/config.yaml and return built-in defaults. Set BEFORE
...
def cmd_gateway(args):
    ...
    from kora_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from kora_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    session_dir = get_kora_home() / "whatsapp" / "session"
...
def cmd_setup(args):
    ...
    from kora_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from kora_cli.config import stamp_install_method
    from kora_cli.dep_ensure import ensure_dependency
...
def select_provider_and_model(args=None):
    ...
    from kora_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    # a leftover OPENAI_BASE_URL in ~/.kora/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.kora/.env if the active provider is not 'custom'.
    ...
    """
    from kora_cli.config import get_env_value, save_env_value, load_config
...
def _aux_select_for_task(task: str) -> None:
    ...
    """
    from kora_cli.config import load_config
    from kora_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from kora_cli.auth import _prompt_model_selection
    from kora_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from kora_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from kora_cli.config import get_env_value

    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.kora/.env by hand. The
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.kora/.env by hand.
    ...
    from kora_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from kora_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from kora_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from kora_cli.codex_models import get_codex_model_ids
    ...
    if not _codex_token:
        try:
            from kora_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    from kora_cli.models import _PROVIDER_MODELS
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from kora_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from kora_cli.auth import _save_model_choice, deactivate_provider
    from kora_cli.config import get_env_value, load_config, save_config
    ...
    from kora_cli.models import probe_api_models
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from kora_cli.runtime_provider import _detect_api_mode_for_url
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from kora_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from kora_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from kora_cli import azure_detect
    import getpass
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from kora_cli.curses_ui import flush_stdin
...
def _model_flow_copilot(config, current_model=""):
    ...
    from kora_cli.config import save_env_value, load_config, save_config
    from kora_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from kora_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from kora_cli.copilot_auth import validate_copilot_token
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    ...
    recover from a malformed paste without editing ``~/.kora/.env`` by hand.
    ...
    from kora_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from kora_cli.config import save_env_value
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "lmstudio":
        from kora_cli.auth import AuthError
        from kora_cli.models import fetch_lmstudio_models
        ...
        from kora_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from kora_constants import display_kora_home as _dhh_fn
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from kora_cli.auth import get_anthropic_key
...
def cmd_login(args):
    ...
    from kora_cli.auth import login_command
...
def cmd_auth(args):
    ...
    from kora_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from kora_cli.status import show_status
...
def cmd_webhook(args):
    ...
    from kora_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    if sub == "manifest":
        from kora_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from kora_cli.kanban import kanban_command
...
def cmd_doctor(args):
    ...
    from kora_cli.doctor import run_doctor
...
def cmd_dump(args):
    ...
    from kora_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from kora_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from kora_cli.config import config_command
...
def cmd_backup(args):
    ...
    if getattr(args, "quick", False):
        from kora_cli.backup import run_quick_backup
...
def cmd_import(args):
    ...
    from kora_cli.backup import run_import
...
def cmd_version(args):
    ...
    try:
        from kora_cli.banner import check_for_updates
        from kora_cli.config import recommended_update_command
...
def cmd_migrate_hermes_home(args):
    """Delegate to the kora_cli.migrate_hermes_home module."""
    from kora_cli.migrate_hermes_home import main as _migrate_main
    argv = []
    if getattr(args, "migrate_check", False):
        argv.append("--check")
    if getattr(args, "migrate_symlink", False):
        argv.append("--symlink")
    if getattr(args, "migrate_copy", False):
        argv.append("--copy")
    if getattr(args, "migrate_force", False):
        argv.append("--force")
    if getattr(args, "migrate_from", None):
        argv.extend(["--from", args.migrate_from])
    if getattr(args, "migrate_to", None):
        argv.extend(["--to", args.migrate_to])
    raise SystemExit(_migrate_main(argv))


def cmd_uninstall(args):
    """Uninstall Hermes Agent."""
    _require_tty("uninstall")
    from kora_cli.uninstall import run_uninstall
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``kora_cli/web_dist/`` (per vite.config.ts
    outDir: "../kora_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "kora_cli" / "web_dist"
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    # build path usable on a stock `py -m kora_cli.main web` invocation.
...
def _find_stale_dashboard_pids() -> list[int]:
    ...
    patterns = [
        "hermes dashboard",
        "kora_cli.main dashboard",
        "kora_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # with `kora_cli.gateway._scan_gateway_pids` and avoids the
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_kora_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_kora_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _invalidate_update_cache():
    ...
    default_home = get_default_kora_root()
...
def _update_node_dependencies() -> None:
    ...
    for label, path in paths:
        ...
        # streamed output to ``~/.kora/logs/update.log`` so nothing is lost.
...
class _UpdateOutputStream:
    ...
    * Every write is also mirrored to an append-only log file
      (``~/.kora/logs/update.log``) that users can inspect after the
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.kora/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    try:
        ...
        # kora_cli.config.get_kora_home to simulate setup failure.
        from kora_cli.config import get_kora_home as _get_kora_home

        logs_dir = _get_kora_home() / "logs"
...
def _cmd_update_check():
    ...
    from kora_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "pip":
        from kora_cli.config import recommended_update_command
        from kora_cli.banner import check_via_pypi
...
def _run_pre_update_backup(args) -> None:
    ...
    try:
        from kora_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_kora_home so the user sees ~/.kora/...
    try:
        from kora_constants import get_kora_home, display_kora_home

        home = get_kora_home()
        try:
            display_path = f"{display_kora_home()}/{out_path.relative_to(home)}"
        except ValueError:
...
def cmd_update(args):
    ...
    """
    from kora_cli.config import is_managed, managed_error
...
def _cmd_update_pip(args):
    ...
    from kora_cli import __version__
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        try:
            from kora_cli.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in kora_cli/config.py bricked
        ...
        # the old bytecode (e.g. get_kora_home added to kora_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload kora_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_kora_home() added since the last release.
        try:
            import importlib
            import kora_constants as _hc
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from kora_cli.tools_config import install_cua_driver
        ...
        if gateway_mode:
            _exit_code_path = get_kora_home() / ".update_exit_code"
            try:
...
def cmd_profile(args):
    ...
    from kora_constants import display_kora_home
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.kora)")
            else:
        ...
        from kora_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from kora_constants import get_kora_home as _hh
        ...
        from kora_cli import profile_describer as _pd
        ...
        from kora_cli.profiles import profile_exists
        ...
        from kora_cli.profiles import rename_profile
        ...
        from kora_cli.profiles import import_profile
        ...
        import tempfile
        from kora_cli.profile_distribution import (
            plan_install,
        ...
        from kora_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        from kora_cli.profile_distribution import describe_distribution, DistributionError
...
def cmd_dashboard(args):
    ...
    if "HERMES_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            ...
            else PROJECT_ROOT / "kora_cli" / "web_dist"
        )
...
def cmd_completion(args, parser=None):
    ...
    from kora_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    ...
    from kora_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from kora_cli.models import CANONICAL_PROVIDERS as _cp
...
# the top-level flags declared in ``kora_cli/_parser.py``.
#
...
def main():
    ...
    try:
        from kora_cli.stdio import configure_windows_stdio
    ...
    from kora_cli._parser import build_top_level_parser
    ...
    # =========================================================================
    from kora_cli.fallback_cmd import cmd_fallback
    ...
    # =========================================================================
    from kora_cli.send_cmd import register_send_subparser
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.kora/scripts/. Default mode: "
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.kora/scripts/. Pass empty string to clear. "
    ...
    # =========================================================================
    from kora_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.kora/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.kora/shell-hooks-allowlist.json."
        ),
    )
    ...
    # =========================================================================
    # migrate-hermes-home command (KR-1 ST3 — ~/.hermes → ~/.kora migration)
    # =========================================================================
    migrate_parser = subparsers.add_parser(
        "migrate-hermes-home",
        help="Migrate the legacy ~/.hermes install dir to ~/.kora",
        description=(
            "Migrate the legacy ~/.hermes install dir to ~/.kora. "
            "Idempotent; safe to run repeatedly. With no mode flag, runs "
            "--check (report-only). Use --symlink for the lowest-friction "
            "transition, --copy if you want to keep ~/.hermes as a "
            "rollback safety net."
        ),
    )
    ...
    migrate_mode.add_argument(
        ...
        dest="migrate_copy",
        action="store_true",
        help="Deep-copy ~/.hermes to ~/.kora (keeps legacy for rollback).",
    )
    migrate_parser.add_argument(
        "--force",
        dest="migrate_force",
        action="store_true",
        help="Replace ~/.kora if it already exists.",
    )
    migrate_parser.add_argument(
        "--from",
        dest="migrate_from",
    ...
    migrate_parser.set_defaults(func=cmd_migrate_hermes_home)
...
""",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.kora/checkpoints/",
    ...
    from kora_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    def cmd_pairing(args):
        from kora_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.kora/skills/.bundled_manifest) "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from kora_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from kora_cli.skills_hub import skills_command
    ...
    from kora_cli.bundles import register_cli as _bundles_register, bundles_command
    ...
    def cmd_plugins(args):
        from kora_cli.plugins_cmd import plugins_command
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from kora_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from kora_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            mem_dir = get_kora_home() / "memories"
            ...
            from kora_cli.memory_setup import memory_command
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            from kora_cli.tools_config import tools_disable_enable_command
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_kora_home() / "sessions"
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.kora/ (by default a "
        "single restore-point archive is written to ~/.kora/backups/ "
    ...
    def cmd_claw(args):
        from kora_cli.claw import claw_command
    ...
    acp_parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.kora/node/ "
...
""",
    ...
    from kora_cli.config import get_container_exec_info
    ...
    if getattr(args, "oneshot", None):
        from kora_cli.oneshot import run_oneshot
