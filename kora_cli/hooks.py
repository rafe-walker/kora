...
Consent records live under ``~/.kora/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.kora/config.yaml``
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.kora/config.yaml.")
