...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m kora_cli.main gateway run --replace`` invocation) and
...
def _current_profile_cli_args() -> list[str]:
    ...
    from kora_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "kora_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def get_task_name() -> str:
    ...
    """
    _assert_windows()
    # Local import to avoid circular module initialization during kora_cli boot.
    from kora_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    script_dir = Path(get_kora_home()) / "gateway-service"
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - invokes ``pythonw -m kora_cli.main [--profile X] gateway run``
    ...
    # if someone imports kora_constants-based logic during startup.
    venv_dir = str(Path(python_path).resolve().parent.parent)
    lines.append(f'set "VIRTUAL_ENV={venv_dir}"')

    pythonw_path = _derive_venv_pythonw(python_path)
    prog_args = [pythonw_path, "-m", "kora_cli.main"]
    if profile_arg:
...
def _write_task_script() -> Path:
    ...
    hermes_home = str(Path(get_kora_home()).resolve())
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    argv = [python_exe, "-m", "kora_cli.main"]
    if profile_arg:
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m kora_cli.main gateway run``
    ...
    log_dir = Path(get_kora_home()) / "logs"
...
def _prompt_install_choices(
    ...
    from kora_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    from kora_cli.gateway import find_gateway_pids, _profile_arg
...
def _wait_for_gateway_ready(timeout_s: float = 6.0, interval_s: float = 0.4) -> list[int]:
    ...
    """
    from kora_cli.gateway import find_gateway_pids
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from kora_cli.config import get_kora_home
        print(f"    type {Path(get_kora_home()).resolve()}\\logs\\gateway.log")
        print(f"    type {Path(get_kora_home()).resolve()}\\logs\\gateway-stdio.log")
...
def stop() -> None:
    ...
    _assert_windows()
    from kora_cli.gateway import kill_gateway_processes
