...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    We delegate to ``kora_cli.config._sanitize_env_lines`` which
    ...
    if not path.exists():
        return
    try:
        from kora_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_hermes_dotenv(
    *,
    ...
    Behavior:
    - `~/.kora/.env` overrides stale shell-exported values when present.
    ...
    home_path = Path(hermes_home or os.getenv("HERMES_HOME", Path.home() / ".kora"))
