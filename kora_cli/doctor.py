...
from kora_cli.config import get_project_root, get_kora_home, get_env_path
...
_DHH = display_kora_home()  # user-facing display path (e.g. ~/.kora or ~/.kora/profiles/coder)

# Load environment variables from ~/.kora/.env so API key checks work
_env_path = get_env_path()
load_hermes_dotenv(hermes_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from kora_cli.colors import Colors, color
from kora_cli.models import _HERMES_USER_AGENT
from kora_cli.vercel_auth import describe_vercel_auth
from kora_constants import OPENROUTER_MODELS_URL
...
from kora_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.kora/.env contains provider auth/base URL settings."""
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from kora_cli.auth import get_minimax_oauth_auth_status
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from kora_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.kora/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    # Check ~/.kora/.env (primary location for user config)
    ...
    # Check ~/.kora/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from kora_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from kora_cli.providers import (
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from kora_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.kora/.env or run 'hermes setup')",
                            (
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from kora_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from kora_cli.config import check_config_version, migrate_config
    ...
    try:
        from kora_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
