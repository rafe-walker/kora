...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    ``~/.kora`` symlink that happens to live under ``/private/var/folders``
...
def _build_hermes_tools_mcp_entry() -> dict:
    ...
    # (not get_kora_home()) on purpose: when the env var is unset we want
    ...
    out: dict[str, Any] = {
        ...
        "args": ["-m", "agent.transports.kora_tools_mcp_server"],
    }
    if env:
        out["env"] = env
...
def migrate(
    ...
    *,
    ...
    Args:
        hermes_config: full ~/.kora/config.yaml dict
    ...
    # The server itself is agent/transports/kora_tools_mcp_server.py
