...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    if not any(platforms.values()):
        ...
        print("channel discovery can populate ~/.kora/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_hermes_env() -> None:
    """Populate ``os.environ`` from ``~/.kora/.env`` AND bridge top-level
    ...
    1. ``load_dotenv(~/.kora/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.kora/config.yaml`` into
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.kora/.env and ~/.kora/config.yaml into os.environ so the
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            ...
            "credentials (~/.kora/.env + ~/.kora/config.yaml) — no LLM, "
