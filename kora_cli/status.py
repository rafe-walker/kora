...
from kora_cli.auth import AuthError, resolve_provider
from kora_cli.colors import Colors, color
from kora_cli.config import get_env_path, get_env_value, get_kora_home, load_config
from kora_cli.models import provider_label
from kora_cli.nous_subscription import get_nous_subscription_features
from kora_cli.runtime_provider import resolve_requested_provider
...
def show_status(args):
    ...
    if _effective_provider_label() == "LM Studio":
        from kora_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    jobs_file = get_kora_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_kora_home() / "sessions" / "sessions.json"
