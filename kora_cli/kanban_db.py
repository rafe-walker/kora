...
In standard installs ``<root>`` is ``~/.kora``. In Docker / custom
deployments where ``HERMES_HOME`` points outside ``~/.kora`` (e.g.
...
def kanban_home() -> Path:
    ...
    2. ``get_default_kora_root()``, which already returns ``<root>``
    ...
    * Neither → :func:`kanban_db_path` resolves via
    ...
    try:
        ...
        with _INIT_LOCK:
            ...
            # See kora_state._WAL_INCOMPAT_MARKERS for detection logic.
            from kora_state import apply_wal_with_fallback
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from kora_cli.profiles import normalize_profile_name
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    if not rows:
        return False
    try:
        from kora_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def _resolve_hermes_argv() -> list[str]:
    ...
    3. ``sys.executable -m kora_cli.main`` — fallback for setups where
    ...
    local (not imported from gateway) because ``kora_cli`` sits below
...
def _kanban_worker_skill_available(hermes_home: Optional[str]) -> bool:
    ...
    # home (``~/.kora``), which ships the bundled skill.
    base = _Path(hermes_home) if hermes_home else (_Path.home() / ".kora")
...
def _default_spawn(
    ...
    *,
    ...
    # _apply_profile_override() runs *before* kora_constants is imported.
    # If HERMES_HOME is absent from the child's env, get_kora_home() falls
    # back to Path.home() / ".kora" (the DEFAULT profile root), ignoring the
    ...
    # dispatcher's. Belt-and-braces with the `get_default_kora_root()`
...
def list_profiles_on_disk() -> list[str]:
    ...
    ``kora_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from kora_constants import get_default_kora_root
        default_root = get_default_kora_root()
