...
- Keep data: Remove code but keep ~/.kora/ (configs, sessions, logs)
"""
...
def remove_wrapper_script():
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains kora_cli reference)
                content = wrapper.read_text()
                if 'kora_cli' in content or 'hermes-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from kora_cli.gateway import get_launchd_plist_path
...
def _is_default_hermes_home(hermes_home: Path) -> bool:
    ...
    try:
        ...
        return hermes_home.resolve() == get_default_kora_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    #    Use `python -m kora_cli.main` so we don't depend on a `hermes`
    #    wrapper that may be half-removed mid-uninstall.
    hermes_invocation = [_sys.executable, "-m", "kora_cli.main", "--profile", name]
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.kora/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.kora/ for future reinstall
    """
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.kora/, just remove the hermes-agent subdir
    ...
    # 5. Optionally remove ~/.kora/ data directory (and named profiles)
    if full_uninstall:
