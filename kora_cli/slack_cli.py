...
def _build_full_manifest(bot_name: str, bot_description: str) -> dict:
    ...
    pasting.
    """
    from kora_cli.commands import slack_app_manifest
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``kora_cli/main.py``):
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("HERMES_HOME") or str(Path.home() / ".kora")) / "slack-manifest.json"
        else:
