...
# display_kora_home is imported lazily at call sites to avoid ImportError
# when kora_constants is cached from a pre-update version during `hermes update`.
from kora_cli.setup import (
    print_header, print_info, print_success, print_warning, print_error,
    prompt, prompt_choice, prompt_yes_no,
)
from kora_cli.colors import Colors, color
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    patterns = [
        ...
        "kora_cli.main --profile",
        "kora_cli.main -p",
        "kora_cli/main.py gateway",
        "kora_cli/main.py --profile",
        "kora_cli/main.py -p",
        "hermes gateway",
        "gateway/run.py",
    ]
    current_home = str(get_kora_home().resolve())
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "kora_cli.main"]
    if profile != "default":
...
def _sync_hermes_home_from_systemd_unit(system: bool) -> None:
    ...
    :func:`get_kora_home` falls back to ``/root/.hermes`` — the wrong
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    from kora_constants import is_container
...
def _gateway_list() -> None:
    ...
    """
    try:
        from kora_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
from kora_constants import is_container, is_termux, is_wsl
...
def _profile_suffix() -> str:
    ...
    home = get_kora_home().resolve()
    default = get_default_kora_root().resolve()
    if home == default:
        return ""
...
def _profile_arg(hermes_home: str | None = None) -> str:
    ...
    For ``~/.kora/profiles/<name>``, returns ``"--profile <name>"``.
    ...
    Args:
        hermes_home: Optional explicit HERMES_HOME path. Defaults to the current
            ``get_kora_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from kora_constants import get_default_kora_root
    home = Path(hermes_home or str(get_kora_home())).resolve()
    default = get_default_kora_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    ...
    Default ``~/.kora`` returns ``hermes-gateway`` (backward compatible).
    Profile ``~/.kora/profiles/coder`` returns ``hermes-gateway-coder``.
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.kora`` → ``ai.hermes.gateway.plist`` (backward compatible).
    Profile ``~/.kora/profiles/coder`` → ``ai.hermes.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.kora/hermes-agent  -> /home/alice/.kora/hermes-agent
...
def _hermes_home_for_target_user(target_home_dir: str) -> str:
    ...
    When installing a system service via sudo, get_kora_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.hermes                    → /home/alice/.hermes
      /root/.kora/profiles/coder     → /home/alice/.kora/profiles/coder
      /opt/custom-hermes               → /opt/custom-hermes  (kept as-is)
    """
    current_hermes = get_kora_home().resolve()
    current_default = (Path.home() / ".kora").resolve()
    target_default = Path(target_home_dir) / ".kora"

    # Default ~/.kora → remap to target user's default
    if current_hermes == current_default:
        return str(target_default)

    # Profile or subdir of ~/.kora → preserve the relative structure
    try:
        relative = current_hermes.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.kora) — keep as-is
...
User={username}
Group={group_name}
ExecStart={python_path} -m kora_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
"""
    ...
    hermes_home = str(get_kora_home().resolve())
    profile_arg = _profile_arg(hermes_home)
...
def launchd_install(force: bool = False):
    ...
    from kora_constants import display_kora_home as _dhh
...
def launchd_status(deep: bool = False):
    ...
    if deep:
        log_file = get_kora_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _all_platforms() -> list[dict]:
    ...
    # User-installed platform plugins under ~/.kora/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_kora_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_whatsapp():
    ...
    from kora_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from kora_cli.dingtalk_auth import dingtalk_qr_auth
...
def _is_service_installed() -> bool:
    ...
    if supports_systemd_services():
        ...
        from kora_cli import gateway_windows
...
def _setup_weixin():
    ...
    print_info("  3. Hermes will store the returned account_id/token in ~/.kora/.env.")
    ...
    import asyncio
    try:
        credentials = asyncio.run(qr_login(str(get_kora_home())))
    except KeyboardInterrupt:
        print()
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``kora_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from kora_cli import setup as _s
    return {
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.kora/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.kora/.env: {', '.join(required)}")
    else:
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            ...
            print("  nohup hermes gateway run > ~/.kora/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    print("  nohup hermes gateway run > ~/.kora/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s hermes 'hermes gateway run'         # persistent via tmux")
                    print("  nohup hermes gateway run > ~/.kora/logs/gateway.log 2>&1 &  # background")
                elif is_windows():
