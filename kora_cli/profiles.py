...
Profiles live under ``~/.kora/profiles/<name>/`` by default.

The "default" profile is ``~/.kora`` itself — backward compatible,
...
_PROFILE_DIRS = [
    ...
    # See kora_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# default profile (``~/.kora``).  Named profiles never contain these
...
# Directories/files to exclude when exporting the default (~/.kora) profile.
...
def _get_profiles_root() -> Path:
    ...
    ``~/.kora``, profiles live under ``HERMES_HOME/profiles/`` so
...
def _get_default_hermes_home() -> Path:
    ...
    In Docker/custom deployments where HERMES_HOME is outside ``~/.kora``
...
def create_profile(
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.kora)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.kora/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    try:
        ...
        from kora_cli.gateway import get_service_name, get_launchd_plist_path
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.kora/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
...
def get_active_profile_name() -> str:
    ...
    Returns ``"default"`` if HERMES_HOME is not set or points to ``~/.kora``.
    Returns the profile name if HERMES_HOME points into ``~/.kora/profiles/<name>``.
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.kora itself — its parent is ~/ and its
        # directory name is ".kora", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    # dir.  Importing as "default" would target ~/.kora itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.kora). "
