...
from kora_cli.nous_subscription import get_nous_subscription_features
...
# display_kora_home imported lazily at call sites (stale-module safety during hermes update)
...
from kora_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from kora_cli.curses_ui import curses_checklist
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    try:
        ...
        from kora_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
        _ensure_plugins()
        _video_backend = None
    ...
    try:
        from kora_cli.auth import get_provider_auth_state
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    from kora_cli.main import select_provider_and_model
    try:
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from kora_cli.auth_commands import auth_add_command
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.kora/.env."
            )
...
def _write_slack_manifest_and_instruct():
    ...
    """
    try:
        from kora_cli.slack_cli import _build_full_manifest
        ...
        target = Path(get_kora_home()) / "slack-manifest.json"
...
def _setup_qqbot():
    ...
    from kora_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def setup_gateway(config: dict):
    ...
    from kora_cli.gateway import _all_platforms, _platform_status, _configure_platform
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``kora_cli.auth`` — lists every supported
    ...
    """
    try:
        from kora_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from kora_cli.config
    ...
    if section_key == "model":
        ...
        from kora_cli.gateway import _all_platforms, _platform_status
