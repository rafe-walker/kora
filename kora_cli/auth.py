...
is persisted in ~/.kora/auth.json with cross-process file locking.
...
from kora_cli.config import get_kora_home, get_config_path, read_raw_config
from kora_constants import OPENROUTER_BASE_URL
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from kora_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Check both os.environ and ~/.kora/.env file
...
# =============================================================================
# Auth Store — persistence layer for ~/.kora/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_kora_home() / "auth.json"
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".kora" / "auth.json").resolve(strict=False)
        try:
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    try:
        ...
        global_root = get_default_kora_root()
    except Exception:
        return None
    profile_home = get_kora_home()
    try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.kora/auth.json`` even when HERMES_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_kora_root()`` for a profile-shaped HERMES_HOME can
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".kora" / "auth.json"
            try:
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from kora_cli.config import validate_config_structure
...
def resolve_provider(
    ...
    *,
    ...
    raise AuthError(
        ...
        "OPENAI_API_KEY, etc.) in ~/.kora/.env.",
...
#
# Tokens live in ~/.kora/auth/google_oauth.json (managed by agent.google_oauth).
...
# Spotify auth — PKCE tokens stored in ~/.kora/auth.json
# =============================================================================
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.kora/.env, and return it.
    ...
    print()
    print("Saved HERMES_SPOTIFY_CLIENT_ID to ~/.kora/.env")
    print()
    return raw
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.kora/auth.json (not ~/.codex/)
#
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Hermes auth store (~/.kora/auth.json).
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to Hermes auth store (~/.kora/auth.json)."""
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.kora/auth.json
# =============================================================================
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.kora/auth.json``. A single MITM during initial
...
# ``get_default_kora_root()`` returns — ``~/.kora`` on Linux/macOS,
...
def _nous_shared_auth_dir() -> Path:
    ...
    :func:`kora_constants.get_default_kora_root` returns — so
    Linux/macOS classic installs land at ``~/.kora/shared/``, native
    ...
    from kora_constants import get_default_kora_root
    return get_default_kora_root() / "shared"
...
def _nous_shared_store_path() -> Path:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from kora_constants import get_default_kora_root
        real_home_shared = (
            get_default_kora_root() / "shared" / NOUS_SHARED_STORE_FILENAME
    ...
    *,
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "~/.kora/auth.json) called POST /api/oauth/token with Hermes's "
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    try:
        from kora_cli.config import load_config, get_env_value
        cfg = load_config()
    except Exception:
        cfg = {}
...
def _prompt_model_selection(
    ...
    """
    from kora_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from kora_cli.config import save_config, load_config
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.kora/auth.json."""
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Hermes auth store (~/.kora/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
