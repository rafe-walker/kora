...
Moved verbatim from ``kora_cli/short_circuit/dm_phrasebook.py``
per KR-PLUGIN-EXTRACTIONS-BATCH-2 (Deliverable C). The shim at
the old location re-exports the public surface so the slack DM
handler, web server endpoints, and phrasebook editor (PR #177)
...
# Phrasebook resolution order
  ...
  2. Bundled ``default_slack_dm_phrasebook.yml`` (package_data at
     ``kora_cli/short_circuit/`` — see ``constants.py``)
...
caller (handler) sees ``None`` from :func:`try_short_circuit`
and proceeds with normal engine resolution.
...
def _operator_override_path() -> Optional[Path]:
    ...
    try:
        ...
        return None
    try:
        candidate = get_kora_home() / OPERATOR_OVERRIDE_RELATIVE
    except Exception:
        return None
...
def _read_bundled_default() -> str:
    ...
    The YAML lives at ``kora_cli/short_circuit/`` (NOT the plugin
    sub-directory) because the phrasebook editor (PR #177)
    reads/writes it through that package. See
    :mod:`...short_circuit.constants` for the source of truth.
    """
    ...
    try:
        ...
        return (
            files(BUNDLED_PHRASEBOOK_PACKAGE)
            .joinpath(BUNDLED_PHRASEBOOK_FILENAME)
