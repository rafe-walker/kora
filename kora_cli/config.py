...
Config files are stored in ~/.kora/ for easy access:
- ~/.kora/config.yaml  - All settings (model, toolsets, terminal, etc.)
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.kora/config.yaml`` causes ``load_config()``
...
from kora_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    managed_marker = get_kora_home() / ".managed"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    Resolution order:
    1. Stamped ``~/.kora/.install_method`` file (written by installers)
    ...
    5. Fallback -> 'pip'
    """
    stamp = get_kora_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.kora/.install_method."""
    stamp = get_kora_home() / ".install_method"
    try:
...
def get_container_exec_info() -> Optional[dict]:
    ...
    container_mode_file = get_kora_home() / ".container-mode"
...
# Re-export from kora_constants — canonical definition lives there.
from kora_constants import get_kora_home  # noqa: F811,E402
...
def ensure_hermes_home():
    """Ensure ~/.kora directory structure exists with secure permissions.
...
DEFAULT_CONFIG = {
    ...
    "checkpoints": {
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.kora/checkpoints/`` size (MB).  When
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.kora/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.kora/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "curator": {
        ...
        # skipped), snapshot ~/.kora/skills/ into
        # ~/.kora/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
    ...
    # stored approval from ~/.kora/shell-hooks-allowlist.json.
    ...
    "security": {
        ...
        # ``kora_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
    ...
    # Logging — controls file logging to ~/.kora/logs/.
    ...
    # Session storage — controls automatic cleanup of ~/.kora/state.db.
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``kora_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("HERMES_HOME", "~/.kora")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            try:
                user_plugins_dir = get_kora_home() / "plugins"
    ...
    #   3. Creates `~/.kora/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_hermes_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_kora_home() / "logs" / "curator"
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.kora/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.kora/config.yaml.
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.kora/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.kora/.env.
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.kora/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.kora/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.kora/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.kora/.env into os.environ. Returns count of vars updated.
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.kora/.env or environment."""
