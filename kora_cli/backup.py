...
`hermes backup` creates a zip archive of the entire ~/.kora/ directory
...
from kora_constants import get_default_kora_root, get_kora_home, display_kora_home
...
def run_backup(args) -> None:
    ...
    hermes_root = get_default_kora_root()
    ...
    # Collect files
    print(f"Scanning {display_kora_home()} ...")
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_kora_home()}")
        ...
        if profiles_dir.is_dir():
            try:
                ...
                # kora_cli.profiles might not be available (fresh install)
...
def _quick_snapshot_root(hermes_home: Optional[Path] = None) -> Path:
    home = hermes_home or get_kora_home()
...
def run_quick_backup(args) -> None:
    ...
    if snap_id:
        ...
        print(f"  {len(snaps)} snapshot(s) stored in {display_kora_home()}/state-snapshots/")
...
def create_pre_update_backup(
    ...
    """
    hermes_root = hermes_home or get_default_kora_root()
    if not hermes_root.is_dir():
        return None
