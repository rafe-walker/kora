...
def _codex_curated_models() -> list[str]:
    ...
    """
    from kora_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
def check_nous_free_tier() -> bool:
    ...
    try:
        from kora_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from kora_cli.model_catalog import get_curated_openrouter_models
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from kora_constants import AI_GATEWAY_BASE_URL
...
def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    ...
    """
    try:
        from kora_cli.auth import resolve_nous_runtime_credentials
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from kora_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from kora_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    ``kora_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         when the env var is set in ``~/.kora/.env``.
    ...
    try:
        from kora_cli.auth import read_credential_pool
        from kora_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from kora_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
...
def _ollama_cloud_cache_path() -> Path:
    ...
    return get_kora_home() / "ollama_cloud_models_cache.json"
