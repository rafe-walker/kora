...
_hermes_profiles() {{
    local profiles_dir="$HOME/.kora/profiles"
...
_hermes_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.kora/profiles" ]]; then
        profiles+=("${{(@f)$(ls $HOME/.kora/profiles 2>/dev/null)}}")
    fi
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        ...
        "    echo default",
        "    if test -d $HOME/.kora/profiles",
        "        ls $HOME/.kora/profiles 2>/dev/null",
        "    end",
        "end",
        "",
