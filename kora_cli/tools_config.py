...
Saves per-platform tool configuration to ~/.kora/config.yaml under
...
from kora_cli.nous_subscription import (
...
from kora_cli.cli_output import (  # noqa: E402 — late import block
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from kora_cli.auth import _read_xai_oauth_tokens
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from kora_cli.plugins import discover_plugins, get_plugin_toolsets
...
from kora_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_kora_home()}/hermes-agent && npm install")
                if result.stderr:
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.kora/config.yaml")
        ...
        # to ~/.kora/.env), then continues straight into PKCE. If they
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    ...
    print()
    from kora_constants import display_kora_home
    print(color(f"  Tool configuration saved to {display_kora_home()}/config.yaml", Colors.DIM))
