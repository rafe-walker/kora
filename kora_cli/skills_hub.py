...
def _existing_categories() -> List[str]:
    """Return sorted subdirectory names under ``~/.kora/skills/`` that look
...
def _prompt_for_category(c: Console, existing: List[str]) -> str:
    ...
    if existing:
        ...
        c.print(
            "[bold]Category[/] [dim](optional — press Enter to install flat at ~/.kora/skills/<name>/)[/]"
        )
    try:
    ...
    if not force and not skip_confirm:
        ...
        if bundle.source == "official":
            c.print(Panel(
                ...
                f"Files will be at: [cyan]{display_kora_home()}/skills/{category + '/' if category else ''}{bundle.name}/[/]",
...
def skills_command(args) -> None:
    """Router for `hermes skills <subcommand>` — called from kora_cli/main.py."""
