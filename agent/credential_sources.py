...
Hermes seeds its credential pool from many places:
    ...
    env:<VAR>     — os.environ / ~/.kora/.env
    claude_code   — ~/.claude/.credentials.json
    hermes_pkce   — ~/.kora/.anthropic_oauth.json
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.kora/.env  → clear it
    ...
    """
    from kora_cli.config import get_env_path, remove_env_value
...
def _remove_hermes_pkce(provider: str, removed) -> RemovalResult:
    """~/.kora/.anthropic_oauth.json is ours — delete it outright."""
    from kora_constants import get_kora_home

    result = RemovalResult()
    oauth_file = get_kora_home() / ".anthropic_oauth.json"
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        ...
        description="~/.kora/.anthropic_oauth.json",
    ))
    register(RemovalStep(
