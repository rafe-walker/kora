...
The version is sourced live from ``kora_cli.__version__`` so it auto-aligns
...
``kora_cli.__version__`` is the canonical source of truth.
"""
...
def _hermes_version() -> str:
    ...
    Falls back to ``"unknown"`` if ``kora_cli`` cannot be imported (should
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=hermes-agent", kora_client_tag()]
