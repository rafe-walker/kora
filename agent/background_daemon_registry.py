"""Registry for plugin-provided background daemons.
...
This module exposes a registration surface for that category so
plugin authors can declare such daemons without forking the
runtime.
...
the gateway/CLI consumer's responsibility. Keeping execution
separate means the API surface can ratify independently of
where the lifecycle code lives.
