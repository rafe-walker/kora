...
from kora_cli.config import get_env_value, load_env
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    try:
        from kora_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
    ...
    if provider == "anthropic":
        ...
        try:
            from kora_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.kora/auth.json providers.minimax-oauth.
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            from kora_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.kora/.env over os.environ — the user's config file is the
    ...
    # won't be re-seeded from the user's shell environment or ~/.kora/.env.
    ...
    try:
        from kora_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
        def _is_source_suppressed(_p, _s):  # type: ignore[misc]
            return False
    if provider == "openrouter":
        # Prefer ~/.kora/.env over os.environ
