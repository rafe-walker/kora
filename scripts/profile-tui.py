...
~30 Hz (matching xterm key-repeat), summarizes ~/.kora/perf.log on exit.
...
bypasses the kora_cli wrapper — we want repeatable timing, not the CLI's
session-picker flow.

Environment overrides:
  HERMES_PERF_LOG     (default ~/.kora/perf.log)
...
try:
    ...
    def get_kora_home() -> Path:  # type: ignore[misc]
...
DEFAULT_LOG = Path(os.environ.get("HERMES_PERF_LOG", str(get_kora_home() / "perf.log")))
DEFAULT_STATE_DB = get_kora_home() / "state.db"
