"""
Timezone-aware clock for the Kora runtime.

Inherited from upstream Hermes (NousResearch/hermes-agent). Provides a
single ``now()`` helper that returns a timezone-aware datetime based
on the user's configured IANA timezone (e.g. ``Asia/Kolkata``).

Resolution order:
  1. ``HERMES_TIMEZONE`` environment variable
  2. ``timezone`` key in ``~/.kora/config.yaml``
...
from kora_constants import get_config_path
