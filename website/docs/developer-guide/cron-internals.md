...
Jobs are stored in `~/.kora/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
```python
# ~/.kora/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.kora/cron/output/` |
