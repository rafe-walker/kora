...
Hermes Agent uses a SQLite database (`~/.kora/state.db`) to persist session
...
db = SessionDB()                           # Default: ~/.kora/state.db
...
Default path: `~/.kora/state.db`

This is derived from `hermes_constants.get_hermes_home()` which resolves to
`~/.kora/` by default, or the value of `HERMES_HOME` environment variable.
