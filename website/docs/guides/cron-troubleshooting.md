...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.kora/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.kora/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.kora/.env` |
...
| `local` | Write access to `~/.kora/cron/output/` |
...
2. `~/.kora/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
...
```bash
ls ~/.kora/scripts/your-script.py   # Must exist
hermes cron edit <job_id> --script ~/.kora/scripts/your-script.py
```
...
Jobs are stored in `~/.kora/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.kora/cron/jobs.json
chmod 600 ~/.kora/cron/jobs.json   # Your user should own it
```
...
2. Check `~/.kora/logs/agent.log` for scheduler messages and `~/.kora/logs/errors.log` for warnings
