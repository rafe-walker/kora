...
What happens:
1. The skill directory is copied to `~/.kora/skills/`
...
```bash
mkdir -p ~/.kora/skills/my-category/my-skill
```

### 2. Write SKILL.md

```markdown title="~/.kora/skills/my-category/my-skill/SKILL.md"
---
name: my-skill
...
The skill appears automatically — no registration needed. Drop it in `~/.kora/skills/` and it's live.
...
**Use categories.** Organize skills into subdirectories (`~/.kora/skills/devops/`, `~/.kora/skills/research/`, etc.). This keeps the list manageable and helps the agent find relevant skills faster.
