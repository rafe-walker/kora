...
The approval system supports three modes, configured via `approvals.mode` in `~/.kora/config.yaml`:
...
Configure the timeout in `~/.kora/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.kora/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.kora/.env` | Overwrite sensitive file via redirection |
...
Commands approved with "always" are saved to `~/.kora/config.yaml`:
...
Set allowed user IDs as comma-separated values in `~/.kora/.env`:
...
Control how unauthorized direct messages are handled in `~/.kora/config.yaml`:
...
Container resources are configurable in `~/.kora/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.kora/sandboxes/docker/<task_id>/`
...
Paths are relative to `~/.kora/`. Files are mounted to `/root/.kora/` inside the container.
...
4. **Store secrets securely** — keep API keys in `~/.kora/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.kora/logs/` for unauthorized access attempts
...
For maximum security, run the gateway on a separate machine or VM. Set `terminal.backend: ssh` in `config.yaml`, then provide host details via environment variables in `~/.kora/.env`:
