...
Copies your current profile's `config.yaml`, `.env`, and `SOUL.md` into the new profile. Same API keys and model, but fresh sessions and memory. Edit `~/.kora/profiles/work/.env` for different API keys, or `~/.kora/profiles/work/SOUL.md` for a different personality.
...
```bash
# Edit coder's tokens
nano ~/.kora/profiles/coder/.env

# Edit assistant's tokens
nano ~/.kora/profiles/assistant/.env
```
...
```bash
coder config set model.default anthropic/claude-sonnet-4
echo "You are a focused coding assistant." > ~/.kora/profiles/coder/SOUL.md
```
...
:::note
You cannot delete the default profile (`~/.kora`). To remove everything, use `hermes uninstall`.
:::
...
The default profile is simply `~/.kora` itself. No migration needed — existing installs work identically.
