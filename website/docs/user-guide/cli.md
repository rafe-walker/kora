...
Every installed skill in `~/.kora/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.kora/config.yaml`:
...
CLI sessions are stored in Hermes's SQLite state database under `~/.kora/state.db`. The database keeps:
