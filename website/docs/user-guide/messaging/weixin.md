...
5. Save the account credentials automatically to `~/.kora/weixin/accounts/`
...
After initial QR login, set at minimum the account ID in `~/.kora/.env`:
...
- Tokens are saved per account+peer to `~/.kora/weixin/accounts/<account_id>.context-tokens.json`
