...
Discord behavior is controlled through two files: **`~/.kora/.env`** for credentials and env-level toggles, and **`~/.kora/config.yaml`** for structured settings. Environment variables always take precedence over config.yaml values when both are set.
...
The `discord` section in `~/.kora/config.yaml` mirrors the env vars above. Config.yaml settings are applied as defaults — if the equivalent env var is already set, the env var wins.
...
**Fix**: Add your User ID to `DISCORD_ALLOWED_USERS` in `~/.kora/.env` and restart the gateway.
...
**Fix**: Set this in `~/.kora/config.yaml` and restart the gateway:
...
```bash
# ~/.kora/.env — works alongside or instead of DISCORD_ALLOWED_USERS
...
```bash
# ~/.kora/.env — env vars win over config.yaml
