...
- A strong shared secret to use as the `clientState` value. Generate with `openssl rand -hex 32` and put it in `~/.kora/.env` as `MSGRAPH_WEBHOOK_CLIENT_STATE`.

## Quick Start

Minimum `~/.kora/config.yaml`:
...
Or via env vars in `~/.kora/.env` (auto-merged on startup):
