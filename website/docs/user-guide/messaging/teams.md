...
Alternatively, configure via `~/.kora/config.yaml`:
...
| `No inference provider configured` | Check that `ANTHROPIC_API_KEY` (or another provider key) is set in `~/.kora/.env` |
...
- Store credentials in `~/.kora/.env` with permissions `600` (`chmod 600 ~/.kora/.env`)
