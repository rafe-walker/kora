...
```bash
mkdir -p ~/.kora/cache/darwinian-evolver && cd ~/.kora/cache/darwinian-evolver
...
```bash
cd ~/.kora/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.kora/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.kora/skills/research/darwinian-evolver
DE_DIR=~/.kora/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Hermes core.
   Custom driver scripts under `~/.kora/skills/...` are user-side and fine.
