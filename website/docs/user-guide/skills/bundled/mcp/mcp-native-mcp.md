...
Add MCP servers to `~/.kora/config.yaml` under the `mcp_servers` key:
...
1. Reads `mcp_servers` from `~/.kora/config.yaml`
...
No `mcp_servers` key in `~/.kora/config.yaml`, or it's empty. Add at least one server.
