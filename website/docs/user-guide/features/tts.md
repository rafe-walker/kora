...
| CLI | Saved to `~/.kora/audio_cache/` | MP3 |
...
tts:
  ...
  piper:
    ...
    # voices_dir: ''                            # default: ~/.kora/cache/piper-voices/
...
On the first TTS call for a voice that isn't cached locally, Hermes runs `python -m piper.download_voices <name>` and downloads the model (~20-90MB depending on quality tier) into `~/.kora/cache/piper-voices/`. Subsequent calls reuse the cached model.
