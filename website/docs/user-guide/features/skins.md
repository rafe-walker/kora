...
/skin mytheme        # switch to a custom skin from ~/.kora/skins/mytheme.yaml
```

Or set the default skin in `~/.kora/config.yaml`:
...
Create YAML files under `~/.kora/skins/`. User skins inherit missing values from the built-in `default` skin, so you only need to specify the keys you want to change.

### Full custom skin YAML template

```yaml
# ~/.kora/skins/mytheme.yaml
...
- Saves directly to `~/.kora/skins/`
- Activates a skin by updating `~/.kora/config.yaml`
...
6. Click **Save** to write the skin YAML to `~/.kora/skins/`.
