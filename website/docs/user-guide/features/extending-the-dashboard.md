...
1. **Themes** — YAML files that repaint the dashboard's palette, typography, layout, and per-component chrome. Drop a file in `~/.kora/dashboard-themes/`; it appears in the theme switcher.
...
Themes are YAML files stored in `~/.kora/dashboard-themes/`. The file name doesn't matter (the theme's `name:` field is what the system uses), but convention is `<name>.yaml`. Every field is optional — missing keys fall back to the built-in `default` theme, so a theme can be as small as one color.

### Quick start — your first theme

```bash
mkdir -p ~/.kora/dashboard-themes
```

```yaml
# ~/.kora/dashboard-themes/neon.yaml
name: neon
label: Neon
...
```yaml
# ~/.kora/dashboard-themes/ocean.yaml
name: ocean
label: Ocean Deep
...
```bash
mkdir -p ~/.kora/plugins/my-plugin/dashboard/dist
```

Write the manifest:

```json
// ~/.kora/plugins/my-plugin/dashboard/manifest.json
{
  "name": "my-plugin",
  "label": "My Plugin",
  "icon": "Sparkles",
...
```javascript
// ~/.kora/plugins/my-plugin/dashboard/dist/index.js
(function () {
  "use strict";
...
```json
// ~/.kora/plugins/session-notes/dashboard/manifest.json
{
  "name": "session-notes",
  "label": "Session Notes",
...
```javascript
// ~/.kora/plugins/session-notes/dashboard/dist/index.js
(function () {
  const SDK = window.__HERMES_PLUGIN_SDK__;
  const { React } = SDK;
...
```python
# ~/.kora/plugins/my-plugin/dashboard/plugin_api.py
...
|----------|-----------|--------------|
| 1 (wins on conflict) | `~/.kora/plugins/<name>/dashboard/` | `user` |
| 2 | `<repo>/plugins/memory/<name>/dashboard/` | `bundled` |
| 2 | `<repo>/plugins/<name>/dashboard/` | `bundled` |
| 3 | `./.kora/plugins/<name>/dashboard/` | `project` — only when `HERMES_ENABLE_PROJECT_PLUGINS` is set |
...
cp hermes-example-plugins/strike-freedom-cockpit/theme/strike-freedom.yaml \
   ~/.kora/dashboard-themes/

# Plugin
cp -r hermes-example-plugins/strike-freedom-cockpit ~/.kora/plugins/
```
...
Check that the file is in `~/.kora/dashboard-themes/` and ends in `.yaml` or `.yml`. Refresh the page. Run `curl http://127.0.0.1:9119/api/dashboard/themes` — your theme should be in the response. If the YAML has a parse error, the dashboard logs to `errors.log` under `~/.kora/logs/`.

**My plugin's tab doesn't show up.**
1. Check the manifest is at `~/.kora/plugins/<name>/dashboard/manifest.json` (note the `dashboard/` subdirectory).
...
4. Tail `~/.kora/logs/errors.log` for `Failed to load plugin <name> API routes` — import errors are logged there.
...
Dashboard plugins are installed by directory layout, not by pip entry point. The cleanest distribution path today is a git repo the user clones into `~/.kora/plugins/`. A pip-based installer for dashboard plugins is not currently wired up.
