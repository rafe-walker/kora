...
|--------|---------------|---------|----------|
| **[Gateway hooks](#gateway-event-hooks)** | `HOOK.yaml` + `handler.py` in `~/.kora/hooks/` | Gateway only | Logging, alerts, webhooks |
...
Each hook is a directory under `~/.kora/hooks/` containing two files:
...
LOG_FILE = Path.home() / ".kora" / "hooks" / "my-hook" / "activity.log"
...
```yaml
# ~/.kora/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.kora/hooks/long-task-alert/handler.py
import os
import httpx
...
```yaml
# ~/.kora/hooks/command-logger/HOOK.yaml
name: command-logger
...
```python
# ~/.kora/hooks/command-logger/handler.py
import json
from datetime import datetime
from pathlib import Path

LOG = Path.home() / ".kora" / "logs" / "command_usage.jsonl"
...
```yaml
# ~/.kora/hooks/session-webhook/HOOK.yaml
...
```python
# ~/.kora/hooks/session-webhook/handler.py
import httpx
...
A popular pattern from the community: drop a Markdown checklist at `~/.kora/BOOT.md`, and have the agent run it once every time the gateway starts. Useful for "on every boot, check overnight cron failures and ping me on Discord if anything failed," or "summarize the last 24h of deploy.log and post it to Slack #ops."
...
1. A file at `~/.kora/BOOT.md` with natural-language startup instructions.
...
Create `~/.kora/BOOT.md`. Write it as if you were giving instructions to a human assistant:
...
**`~/.kora/hooks/boot-md/HOOK.yaml`**

```yaml
name: boot-md
description: Run ~/.kora/BOOT.md on gateway startup
events:
  - gateway:startup
```

**`~/.kora/hooks/boot-md/handler.py`**

```python
"""Run ~/.kora/BOOT.md on every gateway startup."""
...
BOOT_FILE = Path.home() / ".kora" / "BOOT.md"
...
Delete `~/.kora/BOOT.md` to disable the checklist — the hook stays loaded but silently skips when the file isn't there.
...
1. On gateway startup, `HookRegistry.discover_and_load()` scans `~/.kora/hooks/`
...
|-----------|-------------|-------------------------------|---------------------------------------|
| Declared in | `hooks:` block in `~/.kora/config.yaml` | `register()` in a `plugin.yaml` plugin | `HOOK.yaml` + `handler.py` directory |
| Lives under | `~/.kora/agent-hooks/` (by convention) | `~/.kora/plugins/<name>/` | `~/.kora/hooks/<name>/` |
...
hooks:
  post_tool_call:
    - matcher: "write_file|patch"
      command: "~/.kora/agent-hooks/auto-format.sh"
```

```bash
#!/usr/bin/env bash
# ~/.kora/agent-hooks/auto-format.sh
payload="$(cat -)"
...
hooks:
  pre_tool_call:
    - matcher: "terminal"
      command: "~/.kora/agent-hooks/block-rm-rf.sh"
      timeout: 5
```

```bash
#!/usr/bin/env bash
# ~/.kora/agent-hooks/block-rm-rf.sh
payload="$(cat -)"
...
hooks:
  pre_llm_call:
    - command: "~/.kora/agent-hooks/inject-cwd-context.sh"
```

```bash
#!/usr/bin/env bash
# ~/.kora/agent-hooks/inject-cwd-context.sh
...
hooks:
  subagent_stop:
    - command: "~/.kora/agent-hooks/log-orchestration.sh"
```

```bash
#!/usr/bin/env bash
# ~/.kora/agent-hooks/log-orchestration.sh
log=~/.kora/logs/orchestration.log
...
Each unique `(event, command)` pair prompts the user for approval the first time Hermes sees it, then persists the decision to `~/.kora/shell-hooks-allowlist.json`. Subsequent runs (CLI or gateway) skip the prompt.
...
- Keep scripts inside `~/.kora/agent-hooks/` so the path is easy to audit.
