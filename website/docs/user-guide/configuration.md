...
All settings are stored in the `~/.kora/` directory for easy access.
...
2. **`~/.kora/config.yaml`** — the primary config file for all non-secret settings
3. **`~/.kora/.env`** — fallback for env vars; **required** for secrets (API keys, tokens, passwords)
...
**Credential forwarding:** Env vars listed in `docker_forward_env` are resolved from your shell environment first, then `~/.kora/.env`. Skills can also declare `required_environment_variables` which are merged automatically.
...
**Persistence:** When enabled, the sandbox filesystem is snapshotted on cleanup and restored on next session. Snapshots are tracked in `~/.kora/modal_snapshots.json`. This preserves filesystem state, not live processes, PID space, or background jobs.

**Credential files:** Automatically mounted from `~/.kora/` (OAuth tokens, etc.) and synced before each command.
...
**Scratch directory:** Resolved in order: `TERMINAL_SCRATCH_DIR` → `TERMINAL_SANDBOX_DIR/singularity` → `/scratch/$USER/hermes-agent` (HPC convention) → `~/.kora/sandboxes/singularity`.
...
- The remote sandbox may have been torn down by the time you go looking; the local `~/.kora/cache/remote-syncs/…` copy is the authoritative record of what the agent changed.
...
terminal:
  ...
  docker_volumes:
    ...
    - "/home/user/.kora/cache/documents:/output" # Gateway-visible exports
```
...
`/home/user/.kora/cache/documents:/output`.

- Write files inside Docker to `/output/...`
- Emit the **host path** in `MEDIA:`, for example:
  `MEDIA:/home/user/.kora/cache/documents/report.txt`
...
Hermes resolves each listed variable from your current shell first, then falls back to `~/.kora/.env` if it was saved with `hermes config set`.
...
Or via environment variable (in `~/.kora/.env`):
...
:::note
Streaming is disabled by default. Enable it in `~/.kora/config.yaml` to try the streaming UX.
:::
...
quick_commands:
  ...
  update:
    type: exec
    command: cd ~/.kora/hermes-agent && git pull && pip install -e .
  gpu:
    type: exec
...
**Exa:** Set `EXA_API_KEY` in `~/.kora/.env`. Supports `category` filtering (`company`, `research paper`, `news`, `people`, `personal site`, `pdf`) and domain/date filters.
...
browser:
  ...
  record_sessions: false         # Auto-record browser sessions as WebM videos to ~/.kora/browser_recordings/
...
| File | Purpose | Scope |
|------|---------|-------|
| `SOUL.md` | **Primary agent identity** — defines who the agent is (slot #1 in the system prompt) | `~/.kora/SOUL.md` or `$HERMES_HOME/SOUL.md` |
...
```bash
# In ~/.kora/.env or ~/.kora/config.yaml:
