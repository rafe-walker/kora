...
Set your provider with `hermes model` or by editing `~/.kora/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.
...
API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). Hermes Agent does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.kora/`.
...
**Solution:** Hermes auto-sources `~/.bashrc` by default. If that's not enough — e.g. you're a zsh user whose PATH lives in `~/.zshrc`, or you init `nvm` from a standalone file — list the extra files to source in `~/.kora/config.yaml`:
...
:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.kora/.env` for conflicting entries.
:::
...
cat ~/.kora/logs/gateway.log | tail -50
```
...
- Check gateway logs: `cat ~/.kora/logs/gateway.log | tail -50`
...
Configure in `~/.kora/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
nohup hermes gateway run > ~/.kora/logs/gateway.log 2>&1 &
```
...
cd ~/.kora/hermes-agent && uv pip install -e ".[mcp]"
...
Verify your `~/.kora/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
There is no hard limit. Each profile is just a directory under `~/.kora/profiles/`. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
**Solution: Delegation config.** Hermes can route subagents to a different model automatically. Set this in `~/.kora/config.yaml`:
...
2. On the **source machine**, create a full backup:
   ...
   hermes backup
   ```
   This creates a zip of your entire `~/.kora/` directory — config, API keys, memories, skills, sessions, and profiles — saved to your home directory as `~/hermes-backup-<timestamp>.zip`.
...
```bash
rsync -av --exclude='hermes-agent' ~/.kora/ newmachine:~/.kora/
```
