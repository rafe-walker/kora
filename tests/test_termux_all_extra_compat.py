...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    # Project name was renamed from hermes-agent → kora in KR-1; extras
    # are referenced as ``kora[termux]`` now.
    assert '"kora[termux]"' in text
    assert '"kora[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"kora[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
