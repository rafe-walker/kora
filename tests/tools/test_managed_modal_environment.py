...
def _restore_tool_and_agent_modules():
    ...
    original_modules = {
        ...
        if name in {"tools", "agent", "kora_cli"}
        or name.startswith("tools.")
        or name.startswith("agent.")
        or name.startswith("kora_cli.")
    }
    try:
        yield
    finally:
        _reset_modules(("tools", "agent", "kora_cli"))
...
def _install_fake_tools_package(*, credential_mounts=None):
    ...
    kora_cli = types.ModuleType("kora_cli")
    kora_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["kora_cli"] = kora_cli
    sys.modules["kora_cli.config"] = types.SimpleNamespace(
        get_kora_home=lambda: Path(tempfile.gettempdir()) / "hermes-home",
    )
...
def test_managed_modal_rejects_host_credential_passthrough():
    _install_fake_tools_package(
        credential_mounts=[{
            ...
            "container_path": "/root/.kora/token.json",
        }]
    )
    managed_modal = _load_tool_module("tools.environments.managed_modal", "environments/managed_modal.py")
