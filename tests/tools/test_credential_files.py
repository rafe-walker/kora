...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.kora/token.json"
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.kora/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/home/user/.kora/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.kora/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.kora/skills/cat/myskill/scripts/run.sh" in paths
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        assert "/root/.kora/cache/documents" in paths
        assert "/root/.kora/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.kora/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        hermes_home = tmp_path / ".kora"
        hermes_home.mkdir()
        # Use legacy dir name — get_kora_dir prefers old if it exists
        ...
        assert "/root/.kora/cache/documents" in container_paths
        assert "/root/.kora/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        assert entries[0]["container_path"] == "/root/.kora/cache/screenshots/session_abc/screen1.png"
