...
class TestHermesConstantsFallback:
    """Tests for _hermes_home.py fallback when kora_constants is unavailable."""
    ...
    def _load_helper(self, monkeypatch):
        """Load _hermes_home.py with kora_constants blocked."""
        monkeypatch.setitem(sys.modules, "kora_constants", None)
    ...
    def test_fallback_uses_hermes_home_env_var(self, monkeypatch, tmp_path):
        """When kora_constants is missing, HERMES_HOME comes from env var."""
        monkeypatch.setenv("HERMES_HOME", str(tmp_path / "custom-hermes"))
        module = self._load_helper(monkeypatch)
        assert module.get_kora_home() == tmp_path / "custom-hermes"

    def test_fallback_defaults_to_dot_hermes(self, monkeypatch):
        """When kora_constants is missing and HERMES_HOME unset, default to ~/.kora."""
        monkeypatch.delenv("HERMES_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_kora_home() == Path.home() / ".kora"
    ...
    def test_fallback_display_kora_home_shortens_path(self, monkeypatch):
        """Fallback display_kora_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("HERMES_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_kora_home() == "~/.kora"

    def test_fallback_display_kora_home_profile_path(self, monkeypatch):
        """Fallback display_kora_home() handles profile paths under ~/."""
        monkeypatch.setenv("HERMES_HOME", str(Path.home() / ".hermes/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_kora_home() == "~/.kora/profiles/coder"

    def test_fallback_display_kora_home_custom_path(self, monkeypatch):
        """Fallback display_kora_home() returns full path for non-home locations."""
        monkeypatch.setenv("HERMES_HOME", "/opt/hermes-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_kora_home() == "/opt/hermes-custom"

    def test_delegates_to_kora_constants_when_available(self):
        """When kora_constants IS importable, _hermes_home delegates to it."""
        ...
        assert module.get_kora_home is kora_constants.get_kora_home
        assert module.display_kora_home is kora_constants.display_kora_home
