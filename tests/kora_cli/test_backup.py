...
def _make_hermes_tree(root: Path) -> None:
    """Create a realistic ~/.kora directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    (root / "memory_store.db").write_bytes(b"fake-sqlite")
    (root / "kora_state.db").write_bytes(b"fake-state")
...
class TestShouldExclude:
    def test_excludes_hermes_agent(self):
        from kora_cli.backup import _should_exclude
...
class TestBackup:
    def test_creates_zip(self, tmp_path, monkeypatch):
        ...
        # get_default_kora_root needs this
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        ...
        from kora_cli.backup import _validate_backup_zip
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only kora_state.db (old wrong name) is rejected."""
        from kora_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["kora_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        from kora_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".kora"
...
class TestValidation:
    ...
    def test_detect_prefix_hermes(self):
        ...
        import io
        from kora_cli.backup import _detect_prefix
...
class TestBackupEdgeCases:
    def test_nonexistent_hermes_home(self, tmp_path, monkeypatch):
        ...
        fake_home = tmp_path / "nonexistent" / ".kora"
...
class TestProfileRestoration:
    ...
    def test_import_without_profiles_module(self, tmp_path, monkeypatch):
        ...
        import kora_cli.backup as backup_mod
        original_import = __builtins__.__import__ if hasattr(__builtins__, '__import__') else __import__

        def fake_import(name, *a, **kw):
            if name == "kora_cli.profiles":
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from kora_cli.backup import _safe_copy_db
...
class TestQuickSnapshot:
    ...
    def test_restore_config(self, hermes_home):
        from kora_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(hermes_home=hermes_home)
    ...
    def test_restore_nonexistent(self, hermes_home):
        from kora_cli.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", hermes_home=hermes_home) is False

    def test_auto_prune(self, hermes_home):
        from kora_cli.backup import create_quick_snapshot, list_quick_snapshots, _QUICK_DEFAULT_KEEP
    ...
    def test_manual_prune(self, hermes_home):
        from kora_cli.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", hermes_home=hermes_home)
...
class TestRunPreUpdateBackup:
    ...
    def hermes_home(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Bust caches for kora_cli.config + kora_constants so they pick up HERMES_HOME
        for mod in list(__import__("sys").modules.keys()):
            if mod.startswith("kora_cli.config") or mod == "kora_constants":
    ...
    def test_backup_flag_creates_backup(self, hermes_home, capsys):
        ...
        from kora_cli.main import _run_pre_update_backup
    ...
    def test_config_enabled_creates_backup(self, hermes_home, capsys):
        ...
        for mod in list(_sys.modules.keys()):
            if mod.startswith("kora_cli.config"):
...
class TestPreMigrationBackup:
    ...
    ``hermes claw migrate`` runs before mutating ~/.kora/."""
    ...
    def test_creates_backup_under_backups_dir(self, hermes_home):
        from kora_cli.backup import create_pre_migration_backup
    ...
    def test_restorable_with_hermes_import(self, hermes_home, tmp_path):
        ...
        from kora_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(hermes_home=hermes_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_missing_hermes_home_returns_none(self, tmp_path):
        """Fresh install with no ~/.kora yet — nothing to back up."""
        from kora_cli.backup import create_pre_migration_backup
    ...
    def test_does_not_touch_pre_update_backups(self, hermes_home):
        ...
        from kora_cli.backup import create_pre_update_backup, create_pre_migration_backup
