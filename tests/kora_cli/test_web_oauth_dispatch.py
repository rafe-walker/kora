"""Regression tests for the OAuth dispatcher in kora_cli.web_server.
...
def test_minimax_login_does_not_launch_anthropic_flow():
    ...
    with patch(
        "kora_cli.auth._minimax_request_user_code",
        return_value=fake_user_code_resp,
    ), patch(
        "kora_cli.auth._minimax_pkce_pair",
...
def test_nous_dashboard_device_flow_honors_legacy_scope_override(monkeypatch):
    ...
    from kora_cli import web_server as ws
...
def test_minimax_dashboard_poller_accepts_absolute_ms_expired_in():
    ...
    try:
        with patch(
            "kora_cli.auth._minimax_poll_token",
            return_value={
                "status": "success",
                "access_token": "access",
            ...
            "kora_cli.auth._minimax_save_auth_state",
...
def test_anthropic_pkce_branch_still_works():
    ...
    with patch(
        "kora_cli.web_server._start_anthropic_pkce",
