...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./hermes` should use `kora_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "hermes"
    called = []

    fake_main_module = types.ModuleType("kora_cli.main")

    def fake_main():
        called.append("kora_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "kora_cli.main", fake_main_module)
