...
Complements tests/kora_cli/test_kanban_db.py (schema + CAS atomicity)
and tests/kora_cli/test_kanban_cli.py (end-to-end run_slash).  The
...
def test_list_profiles_on_disk(tmp_path, monkeypatch):
    ...
    named profiles under ~/.kora/profiles/ that contain a config.yaml."""
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.delenv("HERMES_HOME", raising=False)
    profiles = tmp_path / ".kora" / "profiles"
...
def test_parse_duration_accepts_formats():
    from kora_cli.kanban import _parse_duration
...
def test_cli_bulk_complete_with_summary_rejects(kanban_home):
    ...
    # Note: kora_cli.main doesn't propagate sub-command exit codes
    ...
    env = os.environ.copy()
    r = _run(
        [sys.executable, "-m", "kora_cli.main", "kanban",
...
def test_cli_create_on_fresh_home_auto_inits(tmp_path, monkeypatch):
    ...
    r = _sp.run(
        [_sys.executable, "-m", "kora_cli.main", "kanban",
...
def test_check_dispatcher_presence_silent_when_gateway_running(monkeypatch):
    from kora_cli import kanban as kb_cli
...
def test_gateway_dispatcher_watcher_respects_config_flag_off(monkeypatch):
    ...
    import kora_cli.config as _cfg_mod
