...
from kora_cli.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        from kora_cli.auth import _resolve_verify
...
def test_resolve_nous_runtime_credentials_prefers_invoke_jwt_and_mirrors(
    ...
    import kora_cli.auth as auth_mod
...
def test_nous_inference_auth_logs_do_not_include_secret_values(
    ...
    caplog.set_level(logging.INFO, logger="kora_cli.auth")
...
def test_get_nous_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    ...
    """
    from kora_cli.auth import get_nous_auth_status
...
def test_get_nous_auth_status_reports_revoked_refresh_session(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("kora_cli.auth.resolve_nous_runtime_credentials", _boom)
...
def test_refresh_token_persisted_when_mint_returns_insufficient_credits(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("kora_cli.auth._refresh_access_token", _fake_refresh_access_token)
    monkeypatch.setattr("kora_cli.auth._mint_agent_key", _fake_mint_agent_key)
...
def test_terminal_refresh_failure_quarantines_tokens(
    ...
    from kora_cli import auth as auth_mod
...
class TestLoginNousSkipKeepsCurrent:
    ...
    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        ...
        import kora_cli.auth as auth_mod
        import kora_cli.models as models_mod
        import kora_cli.nous_subscription as ns
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        ...
        import argparse
        import yaml
        from kora_cli.auth import PROVIDER_REGISTRY, _login_nous
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    """
    from kora_cli.auth import persist_nous_credentials, NOUS_DEVICE_CODE_SOURCE
...
def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    ...
    writing to the user's real ``~/.kora/shared/`` across CI runs.
    """
    from kora_cli.auth import _nous_shared_store_path
...
def test_shared_store_honors_env_override(tmp_path, monkeypatch):
    ...
    from kora_cli.auth import _nous_shared_store_path, NOUS_SHARED_STORE_FILENAME
...
def test_shared_store_read_missing_returns_none(shared_store_env):
    ...
    from kora_cli.auth import _read_shared_nous_state
...
def test_shared_store_read_malformed_returns_none(shared_store_env):
    ...
    from kora_cli.auth import _nous_shared_store_path, _read_shared_nous_state
