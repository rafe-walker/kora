...
def test_recommended_update_command_defaults_to_hermes_update(monkeypatch):
    ...
    # ambient ~/.kora/.managed if a prior test left HERMES_HOME pointing
    # somewhere with that marker, which would make get_managed_update_command()
    # return "Update your Nix flake input ..." instead of falling through to
    # detect_install_method().
    with patch("kora_cli.config.get_managed_update_command", return_value=None), \
         patch("kora_cli.config.detect_install_method", return_value="git"):
...
def test_cmd_update_blocks_managed_homebrew(monkeypatch, capsys):
    ...
    with patch("kora_cli.main.subprocess.run") as mock_run:
