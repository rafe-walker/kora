"""Tests for the Kanban DB layer (kora_cli.kanban_db)."""
...
def test_stale_claim_reclaimed(kanban_home, monkeypatch):
    import signal
    import kora_cli.kanban_db as _kb
...
class TestSharedBoardPaths:
    ...
    def test_default_install_anchors_at_home_dot_hermes(
        ...
        # Standard install: HERMES_HOME == ~/.kora, no profile active.
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.kora/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.kora/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.kora/kanban.db.
        default_home = tmp_path / ".kora"
        default_home.mkdir()
    ...
    def test_docker_custom_hermes_home_uses_env_path_directly(
        ...
        # Docker / custom deployment: HERMES_HOME points outside ~/.kora.
        # `get_default_kora_root()` returns env_home directly when it
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # `get_default_kora_root()` walks up to /opt/hermes because
    ...
    def test_hermes_kanban_db_pin_beats_kanban_home(
        ...
        # HERMES_KANBAN_HOME and the `get_default_kora_root()` path.
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see kora_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(kanban_home, caplog):
    ...
    with _patch("kora_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("WARNING", logger="kora_state"):
            conn = kb.connect()
...
def test_resolve_hermes_argv_prefers_path_shim(monkeypatch):
    ...
    import shutil
    import kora_cli.kanban_db as kb
...
def test_resolve_hermes_argv_avoids_implicit_windows_batch_shim(monkeypatch, tmp_path):
    ...
    assert kb._resolve_hermes_argv() == [sys.executable, "-m", "kora_cli.main"]
...
def test_resolve_hermes_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m kora_cli.main`.
    ...
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kb._resolve_hermes_argv()
    assert argv == [sys.executable, "-m", "kora_cli.main"]
...
def test_resolve_hermes_argv_module_actually_runs():
    ...
    sufficient — if `kora_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m kora_cli.main --version`
