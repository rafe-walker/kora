"""Tests for kora_cli.security_advisories.
...
import kora_cli.security_advisories as adv
...
class TestAck:
    ...
    def test_ack_advisory_persists_id(self, isolated_home, monkeypatch):
        ...
        # don't depend on the full kora_cli.config bootstrap.
        store: dict = {"security": {}}
        monkeypatch.setattr(
            "kora_cli.config.load_config", lambda: store
        )
        monkeypatch.setattr(
            "kora_cli.config.save_config",
