"""Tests for Qwen OAuth provider authentication (kora_cli/auth.py).
...
def qwen_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "kora_cli.auth._qwen_cli_auth_path", lambda: creds_path
    )
    return tmp_path
...
def test_resolve_qwen_runtime_credentials_triggers_refresh(qwen_env):
    ...
    with patch(
        "kora_cli.auth._refresh_qwen_cli_tokens", return_value=refreshed
    ) as mock_refresh:
