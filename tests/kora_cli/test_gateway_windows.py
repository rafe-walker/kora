"""Tests for kora_cli.gateway_windows."""

from pathlib import Path

import pytest

import kora_cli.gateway as gateway
import kora_cli.gateway_windows as gateway_windows
import kora_cli.setup as setup
...
def test_build_gateway_argv_uses_base_pythonw_for_uv_venv_launcher(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("kora_cli.config.get_kora_home", lambda: str(tmp_path / "hermes-home"))

    argv, cwd, env_overlay = gateway_windows._build_gateway_argv()

    assert argv[:3] == [str(base_pythonw), "-m", "kora_cli.main"]
