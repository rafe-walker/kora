...
def fresh_home(tmp_path, monkeypatch):
    ...
    # Also reset kora_constants cache so get_default_kora_root() re-reads.
    try:
        import kora_constants
        kora_constants._cached_default_hermes_root = None  # type: ignore[attr-defined]
    except Exception:
        pass
...
def _cli(args: list[str], env_extra: dict | None = None) -> subprocess.CompletedProcess:
    ...
    return subprocess.run(
        [sys.executable, "-m", "kora_cli.main", "kanban"] + args,
        env=env,
        capture_output=True,
        text=True,
        cwd=str(_WORKTREE),
