...
import pytest
from kora_cli import model_switch
...
def test_openrouter_models_replaced_with_live_catalog(monkeypatch):
    ...
    monkeypatch.setattr(model_switch, "list_authenticated_providers",
                        lambda **kw: list(base))
    monkeypatch.setattr("kora_cli.models.fetch_openrouter_models",
...
def test_openrouter_falls_back_to_base_models_on_fetch_failure(monkeypatch):
    ...
    monkeypatch.setattr(model_switch, "list_authenticated_providers",
                        lambda **kw: list(base))
    monkeypatch.setattr("kora_cli.models.fetch_openrouter_models", _raise)
...
def test_current_custom_endpoint_passthrough_marks_current_row(monkeypatch):
    ...
    # via fetch_api_models when an api_key is set. On a dev host with a
    # real Ollama at localhost:11434, that replaces the test's declared
    # models with whatever is installed locally. Stub it to [] so the
    # configured models pass through.
    monkeypatch.setattr("kora_cli.models.fetch_api_models",
                        lambda *a, **kw: [])
