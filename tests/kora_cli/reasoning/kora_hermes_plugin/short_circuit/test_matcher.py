"""Unit tests for the short-circuit phrasebook matcher.

Per KR-PLUGIN-EXTRACTIONS-BATCH-2 (Deliverable C): moved from
``tests/kora_cli/short_circuit/test_dm_phrasebook.py`` to mirror
the canonical code location at
``kora_cli/reasoning/kora_hermes_plugin/short_circuit/matcher.py``.
All test cases preserved verbatim; only the import path updated
to use the canonical location. The backward-compat shim at
``kora_cli/short_circuit/dm_phrasebook.py`` keeps the old import
path working — asserted in the now-small
``tests/kora_cli/short_circuit/test_dm_phrasebook.py``
(shim-verification suite).

Original spec context (KR-CHEAP-TRIVIAL-DM-SHORTCIRCUIT):
