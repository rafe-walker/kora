"""Tests for Codex auth — tokens stored in Hermes auth store (~/.kora/auth.json)."""
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("kora_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr("kora_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("kora_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
