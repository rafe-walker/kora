...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_refreshes_repo_and_tui_node_dependencies(
        ...
        from kora_cli import main as hm
    ...
    def test_update_non_interactive_runs_safe_config_migrations(self, mock_args, capsys):
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "kora_cli.config.get_missing_env_vars", return_value=["MISSING_KEY"]
        ), patch(
            "kora_cli.config.get_missing_config_fields",
            return_value=[{"key": "new.option", "default": True}],
        ), patch("kora_cli.config.check_config_version", return_value=(1, 2)), patch(
            "kora_cli.config.migrate_config",
            return_value={"env_added": [], "config_added": ["new.option"]},
        ), patch("kora_cli.main.sys") as mock_sys:
            ...
            from kora_cli.config import migrate_config
...
class TestCmdUpdateProfileSkillSync:
    ...
    def test_active_profile_included_in_skill_sync(
        ...
        active_p = SimpleNamespace(name="bit", path=Path("/fake/.kora/profiles/bit"))
        other_p = SimpleNamespace(name="work", path=Path("/fake/.kora/profiles/work"))
        ...
        with (
            patch("kora_cli.profiles.list_profiles", return_value=all_profiles),
            patch("kora_cli.profiles.seed_profile_skills", side_effect=fake_seed),
    ...
    def test_single_profile_default_is_synced(
        ...
        with (
            patch("kora_cli.profiles.list_profiles", return_value=[default_p]),
            patch("kora_cli.profiles.seed_profile_skills", side_effect=fake_seed),
            patch("tools.skills_sync.sync_skills", return_value=empty_sync),
        ):
            cmd_update(mock_args)
