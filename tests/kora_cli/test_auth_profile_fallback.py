...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.kora/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.hermes            (has its own auth.json fixture)
    * Profile     -> tmp_path/.kora/profiles/coder   (active, HERMES_HOME points here)
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    ...
    # ~/.kora/profiles/ — get_default_kora_root() returns HERMES_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from kora_cli.auth import read_credential_pool, write_credential_pool
