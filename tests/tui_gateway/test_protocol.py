...
def server():
    with patch.dict("sys.modules", {
        "kora_constants": MagicMock(get_kora_home=MagicMock(return_value="/tmp/hermes_test")),
        "kora_cli.env_loader": MagicMock(),
        "kora_cli.banner": MagicMock(),
        "kora_state": MagicMock(),
    }):
        import importlib
...
def test_slash_exec_handles_plugin_commands_in_live_gateway(server):
    ...
    with patch(
        "kora_cli.plugins.get_plugin_command_handler",
