...
- save provider credentials into `~/.kora/.env`
...
```bash
SCRIPT="$(find ~/.kora/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
3. Buy it and save it into `~/.kora/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
3. persist that number to `~/.kora/.env`
