#!/usr/bin/env python3
"""
SQLite State Store for the Kora runtime.

Inherited from upstream Hermes (NousResearch/hermes-agent). Provides
persistent session storage with FTS5 full-text search, replacing
...
DEFAULT_DB_PATH = get_kora_home() / "state.db"
...
# kora_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``kora_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see kora_cli/kanban_db.py) would
...
class SessionDB:
    ...
    def __init__(self, db_path: Path = None):
        ...
        try:
            ...
            # ``kora_state._set_last_init_error(None)`` explicitly.
