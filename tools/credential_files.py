            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.kora/{rel}"
                        result.append({
...
# Each tuple is (new_subpath, old_name) matching kora_constants.get_kora_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_kora_dir()`` for backward compatibility with old directory layouts.
    """
