...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.kora/config.yaml``. Hermes
...
Configuration is loaded from ~/.kora/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``kora_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from kora_cli.config import get_env_value as _get_env_value
    except ImportError:
...
def _get_default_output_dir() -> str:
    from kora_constants import get_kora_dir
    return str(get_kora_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.kora/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.kora/config.yaml.
    ...
    try:
        ...
        logger.debug("kora_cli.config not available, using default TTS config")
        return {}
    except Exception as e:
...
def _get_piper_voices_dir() -> Path:
    ...
    Resolves to ``~/.kora/cache/piper-voices/`` under the active
    HERMES_HOME so voice downloads follow profile boundaries.
    """
    from kora_constants import get_kora_dir
    root = Path(get_kora_dir("cache/piper-voices", "piper_voices_cache"))
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.kora/config.yaml (tts: section).
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_kora_home()}/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
