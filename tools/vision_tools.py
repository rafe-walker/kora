...
def _resolve_download_timeout() -> float:
    ...
    if env_val:
        try:
            ...
            pass
    try:
        from kora_cli.config import cfg_get, load_config
        cfg = load_config()
...
async def _vision_analyze_native(
    ...
    try:
        ...
        if local_path.is_file():
            ...
            temp_dir = get_kora_dir("cache/vision", "temp_vision_images")
...
async def video_analyze_tool(
    ...
    try:
        ...
        if local_path.is_file():
            ...
            if blocked:
                raise PermissionError(blocked["message"])
            temp_dir = get_kora_dir("cache/video", "temp_video_files")
