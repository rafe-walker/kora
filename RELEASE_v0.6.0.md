...
- **Profile-aware display paths** — all user-facing `~/.kora` paths replaced with `display_hermes_home()` to show the correct profile directory ([#3623](https://github.com/NousResearch/hermes-agent/pull/3623))
...
- **HERMES_HOME for protected paths** — `.env` write-deny path now respects HERMES_HOME instead of hardcoded `~/.kora` ([#3840](https://github.com/NousResearch/hermes-agent/pull/3840))
