...
Uses a file-based lock (~/.kora/cron/.tick.lock) so only one tick
...
# the module) fail with ModuleNotFoundError for kora_time et al.
sys.path.insert(0, str(Path(__file__).parent.parent))

from kora_constants import get_kora_home
from kora_cli._subprocess_compat import windows_hide_flags
from kora_cli.config import load_config, _expand_env_vars
from kora_time import now as _hermes_now
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    if per_job:
        return per_job
    try:
        from kora_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
def _get_kora_home() -> Path:
    """Resolve Hermes home dynamically while preserving test monkeypatch hooks."""
    return _hermes_home or get_kora_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    hermes_home = _get_kora_home()
...
def _job_profile_context(job_id: str, profile: Optional[str]):
    ...
    both point at the resolved profile directory so _get_kora_home(),
    .env/config loading, script resolution, AIAgent construction, and downstream
    get_kora_home() callers agree on the same home.
    ...
    from kora_cli.profiles import normalize_profile_name, resolve_profile_env
    ...
    override_token = None
    try:
        override_token = set_kora_home_override(profile_home)
        ...
        if override_token is not None:
            reset_kora_home_override(override_token)
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    * anything else — run with the current Python interpreter
    ...
    """
    scripts_dir = _get_kora_home() / "scripts"
    ...
    run_env["HERMES_HOME"] = str(_get_kora_home())
    try:
...
def _run_job_impl(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    try:
        ...
        try:
            load_dotenv(str(_get_kora_home() / ".env"), override=True, encoding="utf-8")
        except UnicodeDecodeError:
            load_dotenv(str(_get_kora_home() / ".env"), override=True, encoding="latin-1")
        ...
        try:
            import yaml
            _cfg_path = str(_get_kora_home() / "config.yaml")
